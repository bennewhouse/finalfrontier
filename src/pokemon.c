    ...
    case EVO_MODE_NORMAL:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_LEVEL_NIGHT:
                RtcCalcLocalTime();
                if (GetCurrentTimeOfDay() == TIME_NIGHT < 12 && gEvolutionTable[species][i].param <= level)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            ...
            case EVO_LEVEL_DUSK:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 17 && gLocalTime.hours < 18 && gEvolutionTable[species][i].param <= level) //Change this to TIME_XXXX ????
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL:
            ...
            case EVO_LEVEL_NATURE_AMPED:
                if (gEvolutionTable[species][i].param <= level)
                {
                    u8 nature = GetNature(mon, FALSE); //False to be safe, code is weird
                    switch (nature)
                    {
                    case NATURE_HARDY:
                    case NATURE_BRAVE:
